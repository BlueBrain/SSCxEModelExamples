    ...
    def run(self, cell_model, param_values, sim=None, isolate=None):
        ...
        if (rmp_score <= self.rmp_score_threshold) or (not self.use_rmp_rin_thresholds):
            ...
            if rinhold_response is not None:
                rin = self.rin_efeature.calculate_feature(
                    rinhold_response)

                rin_score = self.rin_efeature.calculate_score(
                    rinhold_response)

                responses.update(rinhold_response)

                if (rin_score <= self.rin_score_threshold) or (not self.use_rmp_rin_thresholds):
                    ...
                    if cell_model.threshold_current is not None:
                        ...
                        if len(self.pre_protocols) > 0:
                            for pre_protocol in self.pre_protocols:
                                ...
                                if live_plot:
                                    plt.plot(
                                        response[
                                            '%s.soma.v' %
                                            pre_protocol.name]['time'],
                                        response[
                                            '%s.soma.v' %
                                            pre_protocol.name]['voltage'])
                                    plt.pause(.1)
                            ...
                            fitcalc = ephys.objectivescalculators.ObjectivesCalculator(pre_objectives)
                            ...
                            for other_protocol in self.other_protocols:
                                ...
                                if live_plot:
                                    plt.plot(
                                        response[
                                            '%s.soma.v' %
                                            other_protocol.name]['time'],
                                        response[
                                            '%s.soma.v' %
                                            other_protocol.name]['voltage'])
                                    plt.pause(.1)
                                responses.update(response)
...
class RatSSCxRinHoldcurrentProtocol(ephys.protocols.Protocol):
    ...
    def run(self, cell_model, param_values, sim, rmp=None):
        ...
        rin_noholding_protocol = self.create_rin_protocol(
            holdi=0)
        rin_noholding_response = rin_noholding_protocol.run(
            cell_model,
            param_values,
            sim=sim)
        rin_noholding = self.rin_efeature.calculate_feature(
    ...
    def create_rin_protocol(self, holdi=None):
        ...
        for recording in rin_protocol.recordings:
            recording.name = recording.name.replace(
                self.rin_protocol_template.name,
                rin_protocol.name)

        rin_protocol.holding_stimulus.step_amplitude = \
            holdi

        return rin_protocol

    def search_holdi(self,
                     cell_model,
                     param_values,
                     sim,
                     holding_voltage,
        ...
        holdi = self.binsearch_holdi(
            ...
            sim,
            upper_bound=0.0,
            lower_bound=self.holdi_estimate_multiplier *
            holdi_estimate,
    ...
    def binsearch_holdi(self,
        ...
        if depth > max_depth:
            ...
            middle_voltage = self.voltage_base(middle_bound,
                                               cell_model,
                                               param_values,
                                               sim=sim)
    ...
    def voltage_base(self, current, cell_model,
                     param_values,
                     sim=None,
                     short=False):
        """
        ...
        if live_plot:
            import pylab
            pylab.plot(
                response[self.prefix + 'Rin.soma.v']['time'],
                response[self.prefix + 'Rin.soma.v']['voltage'])
            pylab.pause(.1)
...
class RatSSCxThresholdDetectionProtocol(ephys.protocols.Protocol):
    ...
    def run(self, cell_model, param_values, sim, holdi, rin, rmp):
        ...
        max_threshold_current = self.search_max_threshold_current(
            rin=rin,
            rmp=rmp)
    ...
    def search_max_threshold_current(self,
                                     rin=None,
                                     rmp=None):
    ...
    def create_step_protocol(self, holdi=0.0, step_current=0.0):
        ...
        for recording in threshold_protocol.recordings:
            recording.name = recording.name.replace(
                self.step_protocol_template.name,
                threshold_protocol.name)

        if threshold_protocol.holding_stimulus is not None:
            threshold_protocol.holding_stimulus.step_amplitude = \
                holdi

        for step_stim in threshold_protocol.step_stimuli:
            step_stim.step_amplitude = \
                step_current
    ...
    def detect_spike(self,
        ...
        if short and self.short_perc < 1.:
        ...
        if live_plot:
            import pylab
            pylab.plot(
                response[self.prefix + 'ThresholdDetection.soma.v']['time'],
                response[self.prefix + 'ThresholdDetection.soma.v']['voltage'])
            pylab.pause(.1)

        return spike_count >= 1

    def binsearch_spike_threshold(self,
                                  cell_model,
                                  param_values,
                                  sim=None,
                                  holdi=None,
    ...
    def search_spike_threshold(self,
                               cell_model,
                               param_values,
                               sim=None,
                               holdi=None,
        ...
        step_currents = numpy.linspace(
            lower_bound,
            upper_bound,
            num=self.short_steps)
...
class StepProtocol(ephys.protocols.SweepProtocol):
    ...
    def __init__(
        ...
        super(StepProtocol, self).__init__(
            ...
            if holding_stimulus is not None else step_stimuli,
    ...
    def run(self, cell_model, param_values, sim=None, isolate=None):
        ...
        responses.update(super(StepProtocol, self).run(
            cell_model,
            param_values,
            sim=sim,
            isolate=isolate))
...
class StepThresholdProtocol(StepProtocol):
    ...
    def run(self, cell_model, param_values, sim=None, isolate=None):
        ...
        responses.update(super(StepThresholdProtocol, self).run(
            cell_model,
            param_values,
            sim=sim,
            isolate=isolate))
